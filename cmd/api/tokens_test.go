...
func TestCreateActivationToken(t *testing.T) {
	...
	tests := []struct {
		...
		{"Activated", "ayo@gmail.com", http.StatusAccepted, []byte("an email will be sent to you containing activation instructions")},
...
func TestPasswordResetToken(t *testing.T) {
	...
	tests := []struct {
		...
		{"Created", "olalekanawe99@gmail.com", http.StatusAccepted, []byte("an email will be sent to you containing password reset instructions")},
		{"InvalidEmail", "gmail.com", http.StatusUnprocessableEntity, []byte("must be a valid email address")},
		{"UnknownEmail", "foo@gmail.com", http.StatusUnprocessableEntity, []byte("no matching email address found")},
		{"InactiveAccount", "ayo@gmail.com", http.StatusUnprocessableEntity, []byte("user account must be activated")},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(http.MethodPost, ts.URL+"/v1/tokens/password", bytes.NewReader(payload))
			if err != nil {
				t.Fatal(err)
			}
